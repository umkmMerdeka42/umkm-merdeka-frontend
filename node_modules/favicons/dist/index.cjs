...
async function createFavicon(sourceset, name, iconOptions) {
  ...
  } else if (path__namespace.extname(name) === ".svg") {
...
class AppleStartupPlatform extends Platform {
  ...
  async createHtml() {
    return this.iconOptions.map(
      (item) => `<link rel="apple-touch-startup-image" media="(device-width: ${item.deviceWidth}px) and (device-height: ${item.deviceHeight}px) and (-webkit-device-pixel-ratio: ${item.pixelRatio}) and (orientation: ${item.orientation})" href="${this.relative(item.name)}">`
    );
  }
}
...
class WindowsPlatform extends Platform {
  ...
  async createHtml() {
    ...
    return [
      ...
      this.iconOptions.find((iconOption) => iconOption.name === tile) ? `<meta name="msapplication-TileImage" content="${this.relative(tile)}">` : "",
  ...
  browserConfig() {
    ...
    for (const { name, ...size } of SUPPORTED_TILES) {
      const icon = this.iconOptions.find((icon2) => hasSize(size, icon2));
      if (icon) {
        tile[name] = {
